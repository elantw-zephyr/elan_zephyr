 ...
 * and does not notify the firmware, a set address patch must be executed on the upper layer
...
enum udc_em32_configuration_state {
    ...
    /* start to do set-configuration patch, send the set-configuration request to upper layer */
...
static void usb_em32_ep_set_halt(struct udc_em32_data *priv, struct udc_ep_config *const cfg, bool isHalt)
{
    uint8_t ep_idx;
