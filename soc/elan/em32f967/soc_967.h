...
static inline bool IS_GPIO_BIT_ACTION(uint32_t action)
{
	return (action == Bit_RESET) || (action == Bit_SET);
}
...
static inline bool IS_GPIO_MODE(uint32_t mode)
{
	return mode == GPIO_Mode_IN || mode == GPIO_Mode_OUT;
}
