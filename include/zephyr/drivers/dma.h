...
static inline int z_impl_dma_request_channel(const struct device *dev, void *filter_param)
{
	int i = 0;
	int channel = -EINVAL;
	const struct dma_driver_api *api = (const struct dma_driver_api *)dev->api;
 ...
 *
 */
__syscall void dma_release_channel(const struct device *dev, uint32_t channel);

static inline void z_impl_dma_release_channel(const struct device *dev, uint32_t channel)
{
	const struct dma_driver_api *api = (const struct dma_driver_api *)dev->api;
	struct dma_context *dma_ctx = (struct dma_context *)dev->data;
 ...
 *
 */
__syscall int dma_chan_filter(const struct device *dev, int channel, void *filter_param);
